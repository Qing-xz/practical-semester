...
## 模型分析阶段
使用 XGBoost 模型对南瓜平均价格进行了预测，并得到了均方误差（MSE）和决定系数（R²）这两个评估指标。
均方误差反映了预测值与真实值之间的偏差程度，数值越小说明预测越准确；
决定系数则体现了模型对数据的拟合优度，越接近 1 表示模型拟合效果越好。

模型在训练集上表现出较低的均方误差（MSE）和较高的决定系数（R²=0.7458），表明其能够较好地拟合训练数据；
但在测试集上，MSE显著增大且R²下降至0.6108，反映出模型存在过拟合现象

改进：利用网格调优重新设置参数；重新选择特征；增加正则化参数
